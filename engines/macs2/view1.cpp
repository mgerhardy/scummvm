...
bool View1::handleInput(const MouseDownMessage &msg) {
	if (msg._button == MouseMessage::MB_LEFT) {
		...
		if (hasPersistentActionBar()) {
			// Enhancement-only strip: cycle verbs only while the strip is visible.
			// Native V2 HUD (macs4win isMacs0200 path): always cycle, including when
			// hideActionBar left MenuMode==0 (overview/world map). Otherwise the
			// post-changeScene Walk cursor sticks and map hotspot scripts never match
			// special 0x01 (Use interaction).
	...
	if (surface != nullptr && !executor->hasScriptError() && sortedCount > 0) {
		...
		for (uint16 local_c = 1; local_c <= sortedCount; local_c++) {
			...
			if (obj->_hasScaling) {
				...
				// PutDAnimStep: exact 2x blit of half-res anim data.
