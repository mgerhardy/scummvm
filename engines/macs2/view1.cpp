...
void View1::updateCursor(const byte *palette) {
	...
	const uint8 *cursorData = g_engine->_imageResources[mode].data();
...
AnimFrame *View1::getInventoryIcon(GameObject *gameObject) {
	...
	result->_data.assign(&blob[offset + 4], &blob[offset + 4] + result->_width * result->_height);
...
void View1::drawBackgroundAnimations(Graphics::ManagedSurface &s) {
	for (int i = 0; i < (int)g_engine->_backgroundAnimations.size(); i++) {
		...
		drawSprite(current._x, current._y, currentFrame._width, currentFrame._height, currentFrame.data(), s, false);
	}
}
...
void View1::drawCurrentSpeaker(Graphics::ManagedSurface &s) {
	...
	drawSprite(pos, frame->_width, frame->_height, frame->data(), s, false);
}
...
void View1::renderString(uint16 x, uint16 y, Common::String s) {
	...
	for (auto iter = s.begin(); iter != s.end(); iter++) {
		...
		if (found) {
			drawSprite(currentX, currentY, data._width, data._height, data.data(), surf, false);
...
void View1::renderStringWithFont(uint16 x, uint16 y, const Common::String &s, const GlyphData *glyphs, uint16 numGlyphs) {
	...
	for (auto iter = s.begin(); iter != s.end(); iter++) {
		...
		for (uint i = 0; i < numGlyphs; i++) {
			if (glyphs[i]._ascii == *iter) {
				drawSprite(currentX, y, glyphs[i]._width, glyphs[i]._height, glyphs[i].data(), surf, false);
...
void View1::drawGlyphs(Macs2::GlyphData *data, int count, uint16 x, uint16 y, Graphics::ManagedSurface &s) {
	...
	for (int i = 0; i < count; i++) {
		...
		drawSprite(currentX, currentY, currentData._width, currentData._height, currentData.data(), s, false);
...
void View1::drawMainMenu(Graphics::ManagedSurface &s) {
	...
	for (int i = 0; i < 9 && i < (int)g_engine->_imageResources.size(); i++) {
		...
		drawSprite(iconX, iconY, frame._width, frame._height, frame.data(), s, false);
...
bool View1::handleInventoryClick(const MouseDownMessage &msg) {
	...
	if (clickedObject != nullptr && g_engine->_scriptExecutor._cursorMode == Script::MouseMode::Use) {
		...
		if (icon != nullptr) {
			...
			g_engine->_imageResources[cursorSlot]._data = icon->_data;
...
void View1::drawInventory(Graphics::ManagedSurface &s) {
	...
	for (int i = 0; i < 6; i++) {
		...
		drawSprite(iconX, iconY, currentFrame._width, currentFrame._height, currentFrame.data(), s, false);
		buttonX += buttonW + 4;
	}
		...
		for (int ix = 0; ix < 5; ix++) {
			...
			if (icon != nullptr) {
				...
				drawSprite(slotW / 2 + itemX - icon->_width / 2,
						   ...
						   icon->_width, icon->_height, icon->data(), s, false);
			}
			itemIndex++;
			itemX += slotW + 4;
		}
...
void View1::drawSprite(int16 x, int16 y, uint16 width, uint16 height, const uint8 *data, Graphics::ManagedSurface &s, bool mirrored, bool useDepth, uint8 depth) {
...
void View1::drawSprite(const Common::Point &pos, uint16 width, uint16 height, const uint8 *data, Graphics::ManagedSurface &s, bool mirrored, bool useDepth, uint8 depth) {
	drawSprite(pos.x, pos.y, width, height, data, s, mirrored, useDepth, depth);
}

void View1::drawSpriteClipped(uint16 x, uint16 y, Common::Rect &clippingRect, uint16 width, uint16 height, const uint8 *data, Graphics::ManagedSurface &s) {
	for (int currentX = 0; currentX < width; currentX++) {
		for (int currentY = 0; currentY < height; currentY++) {
			uint8 val = data[currentY * width + currentX];
			if (val != 0) {
...
void View1::drawSpriteClipped(uint16 x, uint16 y, Common::Rect &clippingRect, const AnimFrame &frame, Graphics::ManagedSurface &s) {
	drawSpriteClipped(x, y, clippingRect, frame._width, frame._height, frame.data(), s);
}

void View1::drawSpriteAdvanced(uint16 x, uint16 y, uint16 width, uint16 height, uint16 scaling, const uint8 *data, Graphics::ManagedSurface &s) {
	int xScaling = 0;
	int yScaling = 0;
...
void View1::drawSpriteAdvanced(const Common::Point &pos, uint16 scaling, const AnimFrame &frame, Graphics::ManagedSurface &s) {
	drawSpriteAdvanced(pos.x, pos.y, frame._width, frame._height, scaling, frame.data(), s);
}

void View1::drawSpriteSuperAdvanced(const Common::Point &pos, const AnimFrame &frame, uint16 scaling, bool mirrored, bool useDepth, uint8 depth, Graphics::ManagedSurface &s, uint8 shadowIntensity) {
	const uint16 &x = pos.x;
	const uint16 &y = pos.y;
	const uint16 &width = frame._width;
	const uint16 &height = frame._height;
...
void View1::drawBorderSide(const Common::Point &pos, const Common::Point &size, Graphics::ManagedSurface &s) {
	...
	while (currentY < clippingRect.bottom) {
		while (currentX < clippingRect.right) {
			drawSpriteClipped(currentX, currentY, clippingRect, sprite._width, sprite._height, sprite.data(), s);
...
void View1::drawImageResources(Graphics::ManagedSurface &s) {
	...
	for (AnimFrame &current : g_engine->_imageResources) {
		...
		drawSprite(Common::Point(x, y), current._width, current._height, current.data(), s, false);
		x += current._width;
...
void View1::drawOriginalSaveLoadPanel(Graphics::ManagedSurface &s) {
	...
	for (int i = 1; i <= 7; i++) {
		...
		drawSprite(iconX, iconY, iconFrame->_width, iconFrame->_height, iconFrame->data(), s, false);
	}
